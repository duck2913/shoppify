...
const enterAnimation = (nex, done, gradient) => {
	const product = nex.querySelector(".image-container");
	const text = nex.querySelector(".showcase-text");
	const circles = nex.querySelectorAll(".circle");
	const arrow = nex.querySelector(".showcase-arrow");
	return (
...
const productPageTransition = function (next, done) {
...
barba.init({
	...
	transitions: [
		...
		// product page transitions
		{
			...
			enter(data) {
				const done = this.async();
				const next = data.container.next;
				productPageTransition(next, done);
			},
		},
	],
});
